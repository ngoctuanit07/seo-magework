...
CREATE TABLE IF NOT EXISTS `{$this->getTable('seosuite_report_category')}` ( 
  ...
  KEY `prepared_name` (`prepared_name`(8)),
  KEY `prepared_meta_title` (`prepared_meta_title`(8)),
  CONSTRAINT `FK_SEOSUITE_PEPORT_CATEGORY` FOREIGN KEY (`entity_id`) REFERENCES `{$this->getTable('catalog_category_entity')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='';
